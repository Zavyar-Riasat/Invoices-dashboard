  ...
  try {
    ...
    if (data.success && data.quote) {
      ...
      const formattedCharges = (quote.additionalCharges || []).map(
        (charge, idx) => ({
          id: charge.id || `charge-${Date.now()}-${idx}`,
      ...
      const formattedDiscounts = (quote.discounts || []).map((discount, idx) => ({
        id: discount.id || `discount-${Date.now()}-${idx}`,
      ...
      setFormData({
        ...
        notes: quote.notes || "",
        termsConditions: quote.termsConditions || "Payment due within 30 days. Prices valid for 30 days.",
      });

      // Set client search to current client name
      setSearchClient(quote.clientName || quote.client?.name || "");
      setLoading(false);
    } else {
